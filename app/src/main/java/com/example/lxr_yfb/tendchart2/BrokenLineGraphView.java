...
public class BrokenLineGraphView extends View {
    ...
    private int textSize = dipToPx(15);//字体大小
    ...
    public BrokenLineGraphView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        brokenHalfPaint.setColor(Color.argb(-1, 255, 140, 0));
        ...
        textYPaint.setTextSize(dipToPx(6));
    ...
    private int getCoordinateY(int i) {
        return (int) (((float) (maxScore - score[i]) / (maxScore - minScore)) * (getYHeight()) + viewHeight * yTopToViewTop);
    }
...
//        if (selectMonth != -1)
    ...
    private void drawBrokenLine(Canvas canvas) {
        ...
        brokenClosePath.moveTo(scorePoints.get(0).x, scorePoints.get(0).y);//从第一个坐标开始画
        for (int i = 0, num = scorePoints.size(); i < num; i++) {
            brokenClosePath.lineTo(scorePoints.get(i).x, scorePoints.get(i).y);
        }
